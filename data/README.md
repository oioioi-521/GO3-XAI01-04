...
| CHNCXR | 16 张原始图(不纳入主线,完整 662 张未下载) | 2 类:结核(_1)/ 正常(_0) | PNG 灰度(L),约 3000×3000 | **文件名后缀即标签**(无需额外标签表) |

ImageNet/VOC 两个子集来自课程网盘 resources 目录(学长毕设项目整理),规模恰好符合本项目的评估集规划(每数据集 300–500 张)。
...
- **状态(2026-09-06 定稿)**:**不纳入主矩阵**。任务书要求数据集 ≥2,主线用 ImageNet + VOC 两数据集(36 单元)即可。
  教师已提供官方完整集下载途径(NLM 数据门户 `data.lhncbc.nlm.nih.gov/public/Tuberculosis-Chest-X-ray-Datasets/Shenzhen-Hospital-CXR-Set/`,
  直链 `openi.nlm.nih.gov/imgs/collections/ChinaSet_AllFiles.zip`,约 3.6 GB,公开无需注册)。
  若后续组内决定启用:下载放入 `raw/` → 运行 `extract_chncxr_labels.py` → 重跑 `sample_eval_set.py` 即可入矩阵。
  当前 16 张原始图仅作定性演示,不参与实验。
