...
def render_report(snapshot: Dict[str, Any], rows: Sequence[Dict[str, Any]], units: Sequence[Dict[str, Any]]) -> str:
    ...
    lines += [
        ...
        "  当前 32 步操作化对小幅输入扰动非常敏感；稳定性 cosine 本身不能证明路径积分是否收敛。",
        "  成本曲线另在单张 ResNet-50 / VOC 图上测得 200 步参考档约 24.7 s/图（约 0.12 s/步），",
        "  但该档未复测忠实性与稳定性，不能据此作 200 步质量判断。",
        "  结论：该操作化在当前配置下最贵且稳定性最低，不建议直接进完整主矩阵；",
        "  若要采用，需按官方仓库（含其指定 VAE/分类器/数据集）复现后重新评估。",
        "- **FourierShap（512 采样）**：单图归因约 0.6–1.0 s，Insertion AUC ≈ 0.05–0.33，稳定性 cosine ≈ 0.66–0.95，",
        "  成本与同采样数的 KernelSHAP 相当或更低；本 OOD 小样本的 Insertion AUC 与 LIME/KernelSHAP 同量级，",
        "  **可作为后续复核候选，但不能据此宣布优于标准方法**。2048 采样约 2.2 s/图。",
        "- 成本曲线见 `frontier_cost_report.md` / `frontier_cost_records.jsonl`。",
        "- 以上均为 `ood_pipeline_diagnostic`，样本仅 8 张/数据集，不构成对论文方法本身的评价。",
        "",
        "## 方法与依据",
        "",
        "- **MA-GIG**：Manifold-Aligned Guided Integrated Gradients，ICML 2026（arXiv:2605.02167）。参考 `cleanig` 的隐空间 Guided-IG 路径：在预训练 VAE 隐空间构造路径，逐步解码；图像空间路径积分。本试跑使用替代权重 `stabilityai/sd-vae-ft-mse`，不视为官方指定 VAE 的等价物。",
