...
和 ``RESULT_INDEX.md``（交付物清单与 P0–P5 状态）。只读取已落盘数据，不占用 GPU。
"""
...
def main() -> int:
    ...
    smoke = pr.read_jsonl(ROOT / "smoke_records.jsonl")
    smoke_ok = [r for r in smoke if r.get("status") == "ok"]
    rise_smoke = pr.read_jsonl(ROOT / "rise_smoke_records.jsonl")
    rise_smoke_ok = [r for r in rise_smoke if r.get("status") == "ok"]
    ...
    index = [
        ...
        "| 阶段 | 状态 | 记录数 | 主要交付物 |",
        "|---|---|---:|---|",
        f"| P0 环境与首通 | 完成（RISE 初次失败后复核通过） | 初次 {len(smoke_ok)}/{len(smoke)}；复核 {len(rise_smoke_ok)}/{len(rise_smoke)} | `environment_snapshot.json`、`smoke_records.jsonl`、`smoke_report.md`、`rise_smoke_*` |",
