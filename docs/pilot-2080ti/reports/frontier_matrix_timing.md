...
生成时间（UTC）：`2026-09-11T15:33:44.440180+00:00`
...
- **FourierShap** 取 512 采样；**MA-GIG** 展示 200 步参考档。后者仅在一张 ResNet-50 / VOC 图上实测归因成本（约 24.7 s），其余 200 步总成本按 32 步矩阵分段外推，以 † 标注。
- 标注 * 的 ImageNet 行为外推：固定 224×224、模型和方法配置后，取同 (模型, 方法) 在两个已测数据集上的均值；仍可能受样本、CPU 回归和系统负载影响。
...
| 前沿 MA-GIG 200 步参考档（分段外推） | 9 | 673.41 | 24.32 小时 | 121.59 小时 | 243.18 小时 |
| 前沿 2 方法合计（MA-GIG 32 步） | 18 | 133.19 | 4.81 小时 | 24.05 小时 | 48.10 小时 |
| **合计 72 条件（MA-GIG 32 步）** | 72 | 213.97 | 7.73 小时 | 38.63 小时 | 77.27 小时 |
| **合计 72 条件（MA-GIG 200 步参考档）** | 72 | 775.53 | 28.01 小时 | 140.03 小时 | 280.05 小时 |
...
- **FourierShap（512 采样）**：单图总耗时约 1.9–3.2 s（与 RISE/KernelSHAP/LIME 同量级）；在本 OOD 小样本协议下，Insertion AUC 与 LIME/KernelSHAP 同量级，稳定性条件均值为 0.66–0.95。它是成本可控的复核候选，但不能据此声称性价比最优。
- **MA-GIG**：本次单图成本随步数大致线性（约 0.12 s/步）。32 步单图总耗时约 12–13 s；200 步参考档实测归因约 24.7 s。其单图总耗时约 75 s、72 条件 500 图约 140.03 小时，后两个数均为分段外推。因此不建议直接进入完整矩阵，可在匹配官方设置的小子集上单独复核。
