...
生成时间（UTC）：`2026-09-11T15:33:44.444546+00:00`
...
| MA-GIG 200 步参考档 | 24.7 s（单图实测） | ~75 s（分段外推） | ~0.12 s/步 |
...
| MA-GIG 200 步参考档（分段外推，9 条件） | 673.4 s | 24.32 小时 | 121.59 小时 |
| 前沿 2 方法合计（MA-GIG 32 步，18 条件） | 133.2 s | 4.81 小时 | 24.05 小时 |
| **合计 72（MA-GIG 32 步）** | 214.0 s | 7.73 小时 | 38.63 小时 |
| **合计 72（MA-GIG 200 步参考档）** | 775.5 s | 28.01 小时 | 140.03 小时 |
...
2. **MA-GIG 不建议直接进入主矩阵**：
   - 200 步参考档单图归因实测约 24.7 s；按“原图归因 + K=2 扰动归因 + 其余开销”分段外推，单图总耗时约 75 s，72 条件 500 图约需 140.03 小时。这两个总量是工程外推，不是 72 条件实测；
