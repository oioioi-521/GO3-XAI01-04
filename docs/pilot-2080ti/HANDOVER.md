...
> 本文档汇总本次试跑（P0–P5 + 外推）的**实测结果、结论、组合爆炸应对方案、局限与待办**，供接手同学复现与继续推进。

---

## 0. 一句话总结

在限时 GPU 预算内验证了整套“方法 × 模型 × 数据集 × 指标”的测量管线：**P2 的 2 数据集 × 3 模型 × 6 方法 × 16 图 = 576 单元全部成功**，两个选做前沿方法的 P5 探针也有 96 单元全部成功。P0 初次 smoke 并非零失败：RISE 因布尔张量双线性上采样报错，修正后专项复核 6/6 通过。成本表可用于工程点估计，但 ImageNet 和 MA-GIG 200 步总成本含外推，不是完整实测。控制组合爆炸的可行方案是“分层矩阵 + 冻结超参 + 包含式递增 + 缓存复用 + K 分级”；PDF 全部 54 条件在每条件 500 图时估计约 14.6 GPU 小时，桌面单卡应预留接近 15 小时并分批断点续跑。前沿方法中，FourierShap 可进入下一轮复核；MA-GIG 当前操作化成本高且 32 步稳定性很低，不建议直接进完整主矩阵。
...
| 阶段 | 内容 | 记录数 | 结果 |
|---|---|---:|---|
| P0 环境与首通 | 版本/权重/数据来源；6 方法首通 | 初次 5/6；专项复核 6/6 | RISE 初次报错，修正后通过；均为合成图首通 |
...
- JSONL 状态重算：P1 54/54、P2 576/576、P3 20/20、P4 24/24、P5 矩阵 96/96、P5 成本曲线 10/10 均为 `ok`。
- `data_manifest.json` 共 32 张真实图；图像文件及 VOC 标注均与清单 SHA256 一致。校核时修正了 `007943.xml` 清单中多写的 1 个十六进制字符，文件本身与官方 tar 内成员一致。
- 派生表行数与协议一致：`eval_per_image.csv` 2304 行、`eval_units.csv` 180 行、`frontier_per_image.csv` 384 行、`frontier_units.csv` 60 行，数值均有限。
- `frontier_matrix_timing.csv` 现与 Markdown 逐条件表统一为 MA-GIG 200 步参考档；该档除单图归因成本外均标记为工程外推。
...
- IG 的 6 个数据集×模型条件均值最低（约 0.35–0.63），RISE 接近 1.0；这些数值只描述当前基线、步数与 OOD 协议，不能单独证明差异来自“梯度噪声”或某一种机制。
- 逐条件数值见 `eval_report.md` / `eval_units.csv`；这是域外管线证据，不能当任务质量结论。

### 5.3 随机方法种子方差（P4，跨种子 cosine）

| 方法 | 8 张图均值 | 各图“跨种子均值”的范围 |
|---|---:|---|
...
**要点**：512→2048 时 RISE / KernelSHAP / LIME 均约增至 4 倍，符合高档区间近似线性；但 KernelSHAP 的 128 样本单点反而慢于 512，说明单图单次曲线受预热/系统波动影响，只适合判断量级。内部 batch 16→32 的加速约 4–5%，建议保持 16 以维持配置一致性和显存余量。
...
|---|---:|---:|---:|---:|---|
| MA-GIG（32 步） | 4.1 s | 0.046 | **0.008** | ~2.4 GiB | 隐空间 Guided-IG；当前操作化对输入扰动高度敏感 |
| FourierShap（512 采样） | 0.76 s | 0.175 | 0.871 | ~2.0–4.5 GiB | 谱代理 + 闭式 Shapley |

**成本曲线**
- MA-GIG：约 **0.12 s/步**；4/8/16/32/64/128/200 步 = 0.53/0.86/1.84/3.94/7.75/15.7/**24.7** s/图。200 步是本次成本曲线的参考高档，仅在单图上测时。
- FourierShap：约 **0.0011 s/采样**；128/512/2048 = 0.23/0.61/2.24 s/图。

**结论**
- **MA-GIG**：32 步下归因成本最高，输入扰动稳定性 cosine≈0.01。这个结果说明当前 OOD 操作化对微小输入变化敏感，但不能单凭 cosine 证明路径积分不收敛。200 步只在单张 ResNet-50 / VOC 图上测了归因成本，未复测忠实性或稳定性。**不建议直接进完整主矩阵，应在任务匹配的小子集复核。**
- **FourierShap**：成本与 RISE / KernelSHAP / LIME 同量级，当前 OOD 小样本中的稳定性条件均值为 0.66–0.95，**可作为后续复核候选**；不能据此宣布优于标准方法。PDF 未给引用，代码依据为 arXiv:2410.06300。
- MA-GIG 参考 ICML 2026（arXiv:2605.02167，官方代码 `leekwoon/ma-gig`）；本试跑 VAE 使用替代权重 `stabilityai/sd-vae-ft-mse`，不视为官方配置等价物。
...
5. **K 与 seed 分级**：主体 K=2；按本次分段计时，K 2→10 会让总成本约增至 3.5 倍。图像数 N 同样线性缩放，因此二者应独立预算；随机方法先测 seed 方差（P4 已测），只对候选加种子。
6. **保持核心比较配对**：所有主体方法先在同一 100 张子集上比较；廉价方法可额外扩到 500 张估计自身方差，但额外 400 张不能直接参与与昂贵方法的配对比较。
...
1. **两个选做前沿方法（已完成试跑级探针，见 §5.6）**：
   - **MA-GIG**：ICML 2026《Manifold-Aligned Guided Integrated Gradients》（arXiv:2605.02167，官方代码 `leekwoon/ma-gig`），`pilot_frontier.py` 为参考论文思路的近似实现。当前 32 步操作化稳定性≈0.01，200 步仅测成本，**不直接进完整主矩阵**。若要正式采用，需按官方仓库（指定 VAE/分类器/数据集）完整复现后再评估。
